...
const getPlatformIP = () => {
  ...
  if (process.env.DEV_SERVER_IP) {
    return process.env.DEV_SERVER_IP;
  }
  
  // Try to detect platform from various sources
  const platform = process.env.EXPO_PLATFORM || 
...
const PLATFORM_IP = getPlatformIP();
 ...
 * for example: if the APP_ENV is staging, the bundle id will be com.yoschools-mobi.staging
 */

// TODO: Replace these values with your own

const BUNDLE_ID = 'com.yoschools.mobi'; // ios bundle id
const PACKAGE = 'com.yoschools.mobi'; // android package name
const NAME = 'YoSchools Mobile'; // app name
const EXPO_ACCOUNT_OWNER = 'expo-owner'; // expo account owner
const EAS_PROJECT_ID = 'c3e1075b-6fe7-4686-aa49-35b46a229044'; // eas project id
const SCHEME = 'yoschools-mobi'; // app scheme
...
const client = z.object({
  ...
  ENABLE_CRASH_REPORTING: z.boolean().default(false),
});
...
const buildTime = z.object({
  ...
  SECRET_KEY: z.string().optional().default('development_secret_key'),
});
...
const _clientEnv = {
  ...
  // YoSchools API Configuration - Let runtime detection handle platform-specific URLs
  API_URL: process.env.API_URL || undefined,
  GRAPHQL_URL: process.env.GRAPHQL_URL || undefined,
  WS_URL: process.env.WS_URL || undefined,
